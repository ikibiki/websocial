...
class Codebird {
    ...
    public function __call($function, $params) {
        // cURL function?
        if (substr($function, 0, 6) === '_curl_' || $function === '_time' || $function === '_microtime'
        ) {
        ...
        return $this->_callApi(
                        $httpmethod, $method, $method_template, $apiparams, $multipart, $app_only_auth
        );
    }
    ...
    protected function _getCurlInitialization($url) {
        ...
        $this->_curl_setopt(
                $connection, CURLOPT_USERAGENT, 'codebird-php/' . $this->getVersion() . ' +https://github.com/jublonet/codebird-php'
        );
    ...
    protected function _getNoCurlInitialization($url, $contextOptions, $hostname = '') {
        ...
        if ($this->_hasProxy()) {
            ...
            if ($this->_getProxyAuthentication()) {
                $httpOptions['header'][] = 'Proxy-Authorization: Basic ' . base64_encode($this->_getProxyAuthentication());
            }
        }
    ...
    protected function _sha1($data) {
        ...
        return base64_encode(hash_hmac(
                        'sha1', $data, self::$_consumer_secret
                        . '&'
    ...
    protected function _sign($httpmethod, $method, $params = []) {
        ...
        $sign_base_params = array_merge(
                $sign_base_params, array_map([$this, '_url'], $params)
        );
    ...
    protected function _detectMethod($method, &$params) {
        ...
        if ($method === 'media/upload' && isset($params['command']) && $params['command'] === 'STATUS'
        ) {
            return 'GET';
        }
    ...
    protected function _buildMultipart($method, $params) {
        ...
        $multipart_request = $this->_getMultipartRequestFromParams($method, $multipart_border, $params)
                . '--' . $multipart_border . '--';
    ...
    protected function _buildBinaryBody($input) {
        if (// is it a file, a readable one?
            ...
            if ($data !== false && strlen($data) !== 0) {
                ...
                filter_var($input, FILTER_VALIDATE_URL) && preg_match('/^https?:\/\//', $input)
        ) {
    ...
    protected function _fetchRemoteFile($url) {
        ...
        if ($this->_use_curl) {
            ...
            $result = $this->_curl_exec($connection);
            if ($result !== false && $this->_curl_getinfo($connection, CURLINFO_HTTP_CODE) === 200
            ) {
                return $result;
            }
        ...
        if ($result !== false && preg_match('/^HTTP\/\d\.\d 200 OK$/', $headers[0])
        ) {
            return $result;
        }
        throw new CodebirdMediaException('Downloading a remote media file failed.');
    ...
    protected function _callApi($httpmethod, $method, $method_template, $params = [], $multipart = false, $app_only_auth = false) {
        if (!$app_only_auth && $this->_oauth_token === null && substr($method, 0, 5) !== 'oauth'
        ) {
    ...
    $httpmethod, $method, $method_template, $params = [], $multipart = false, $app_only_auth = false
        ...
        list ($authorization, $url, $params, $request_headers) = $this->_callApiPreparations(
    ...
    protected function _getBearerAuthorization() {
        if (self::$_consumer_key === null && self::$_bearer_token === null
        ) {
    ...
    $httpmethod, $method, $method_template, $params, $multipart, $app_only_auth
        ...
        if ($httpmethod === 'GET') {
            // GET
            list ($authorization, $url) = $this->_callApiPreparationsGet($httpmethod, $url, $params, $app_only_auth);
        } else {
            // POST
            list ($authorization, $params, $request_headers) = $this->_callApiPreparationsPost($httpmethod, $url, $method, $method_template, $params, $multipart, $app_only_auth);
        }
        if ($app_only_auth) {
    ...
    protected function _parseApiHeaders($reply) {
        ...
        if ($proxy_tester === 'http/1.0 200 connection established' || $proxy_tester === 'http/1.1 200 connection established'
        ) {
            array_shift($reply);
        }
